{
  description = "A workflow-complete rust project on the agentic core";
  ...
  outputs = inputs @ {flake-parts, ...}:
    flake-parts.lib.mkFlake {inherit inputs;} ({config, ...}: {
      ...
      agentic.ciCache.requestedProfiles = ["nix" "rust"];

      perSystem = _: {
        devenv.shells.default = {
          imports = [config.flake.modules.devenv.agentic config.flake.modules.devenv.rust];
